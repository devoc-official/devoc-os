# Database Architecture & Schema — M9 Finance

## Migration Reservation

`009_finance_m9_schema.sql`

All tables use UUID primary keys (`gen_random_uuid()`), tenant scoping (`organization_id`), UTC timestamps (`TIMESTAMPTZ`), exact numeric precision (`NUMERIC(15,4)`), foreign keys, and indexes consistent with M1–M8.

---

## DDL Schema Specifications

### 1. Table: `finance_categories`
```sql
...
### 2. Table: `financial_parties`
```sql
CREATE TABLE financial_parties (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    party_type TEXT NOT NULL CHECK (party_type IN ('person', 'client', 'vendor')),
    person_id UUID REFERENCES people(id) ON DELETE SET NULL,  -- Optional link to M2 Person
    name TEXT NOT NULL,
    email TEXT,
    phone TEXT,
    tax_identifier TEXT,
...
### 4. Table: `financial_obligation_items`
```sql
...
CREATE INDEX idx_financial_parties_org ON financial_parties(organization_id);
CREATE INDEX idx_financial_parties_person ON financial_parties(person_id);

CREATE INDEX idx_financial_obligations_org ON financial_obligations(organization_id);
CREATE INDEX idx_financial_obligations_party ON financial_obligations(party_id);
CREATE INDEX idx_financial_obligations_state ON financial_obligations(state);
CREATE INDEX idx_financial_obligations_due ON financial_obligations(due_at);
CREATE INDEX idx_financial_obligations_project ON financial_obligations(project_id);
...
CREATE INDEX idx_financial_transactions_state ON financial_transactions(state);

CREATE INDEX idx_financial_allocations_trans ON financial_allocations(transaction_id);
CREATE INDEX idx_financial_allocations_obli ON financial_allocations(obligation_id);

CREATE INDEX idx_financial_budgets_org ON financial_budgets(organization_id);
CREATE INDEX idx_financial_budgets_bu ON financial_budgets(business_unit_id);
```

---
*Prepared for Milestone 9 – Finance Engine Schema.*
